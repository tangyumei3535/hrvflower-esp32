...
"""Delegate idf.py actions to esp_board_manager; bootstrap component if missing."""
...
def _download_bmgr_component(project_path: str) -> None:
    ...
    if not manifest_path:
        print(f"Warning: Manifest file not found at {main_dir}")
        return
...
def action_extensions(base_actions: dict, project_path: str) -> dict:
    ...
    if len(sys.argv) > 2 and sys.argv[1] in ("gen-bmgr-config", "bmgr"):
    ...
    bmgr_ext = _load_module(bmgr_path / "idf_ext.py")
    return bmgr_ext.action_extensions(base_actions, project_path)
